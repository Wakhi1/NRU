// Editable right-side drawer for view/create/edit/assignment workflows — the write-capable
// sibling of Drawer (which stays read-only, per the prototype's own reference-panel convention).
// One shared component so every module gets the same RBAC behavior, validation, loading state,
// and toast/close handling instead of reimplementing it per page.
//
// FormDrawer.open({
//   title, sub,
//   readOnly: false,                 // true => every field renders as static text, no Save/Delete
//   sections: [{ label, fields: [
//     { key, label, type: 'text'|'email'|'number'|'date'|'select'|'textarea'|'checkbox', value,
//       options: [{value,label}] | ['a','b'], required, hint, editable (default true) }
//   ]}],
//   extraHtml: '...',                // optional raw HTML appended after the sections (e.g. a member list)
//   afterRender: (drawerEl) => {},   // optional hook to wire listeners — receives the whole drawer element,
//                                    // so it can reach both extraHtml content and section fields (e.g. a
//                                    // person-picker select) by id/data-attribute
...
const FormDrawer = (() => {
  ...
  function renderField(f, readOnly) {
    ...
    if (f.type === 'select') {
      ...
      control = `<input id="${id}" data-fd-field="${f.key}" type="${f.type || 'text'}" value="${esc(f.value != null ? f.value : '')}" ${f.hint && f.type !== 'date' ? `placeholder="${esc(f.hint)}"` : ''} />`;
    }
  ...
  function open(opts) {
    ...
    scrimEl.innerHTML = `
      <div class="drawer" role="dialog" aria-modal="true">
        ...
        ${readOnly ? '<span class="badge badge-neutral">View only — you don\'t have edit access here</span>' : ''}
    ...
    // Pass the whole drawer, not just #fd-extra — a handler often needs to read a section
    // field (e.g. a person picker in `sections`) alongside the injected extraHtml content.
    // IDs/data-attributes inside extraHtml are still unique within this scope, so existing
    // `root.querySelector('#some-injected-id')` callers are unaffected.
