// Directory + dialer + call history (architecture doc §8). The directory lists EVERY active
// employee, not just people who've already opened SPTS — someone with no extension yet shows up
// too (calling them provisions one on the spot, see voip.routes.js's POST /calls). Placing a call
// and the in-call banner itself are handled globally by voip-call.js so a call rings no matter
// which screen answers it — this page is just the "who do I call" list plus a record of what
// already happened.
...
(async () => {
  ...
  document.getElementById('main').innerHTML = `
    <div class="page-head">
      <div><h1>Calls</h1>
        <p class="page-sub">On-net voice over the same data connection as everything else in SPTS — no separate airtime bundle. Calling rings on whatever screen the other person has open.</p></div>
