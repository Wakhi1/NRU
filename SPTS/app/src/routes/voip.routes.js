...
// no separate airtime line." This implements the on-net half of that: handset-to-handset (browser-
// to-browser, for this build) calling over WebRTC, signaled by short polling — the same pattern
// myshift.js already uses for override requests, not a websocket server. Off-net breakout to public
// numbers needs a real SIP trunk provider, a procurement decision outside what this codebase can
// wire up on its own; `call_detail_record.direction` is reserved for that day.
...
// Loads a call the caller or callee is a party to, 404s otherwise — same "don't leak that the id
// even exists" reasoning as scope checks elsewhere in this app (architecture doc §3.4).
async function loadMyCall(callId, employeeNo) {
  ...
  // A ring nobody answered in time reads as missed the next time either party checks on it,
  // rather than needing a background sweep for a call record that's otherwise idle.
...
// Every active employee is callable, not just whoever has already opened SPTS at least once —
// LEFT JOIN so someone with no voip_extension row yet still shows up (with no extension assigned
// and reading as offline); POST /calls already auto-provisions the callee's extension on demand
// (see ensureExtension below), so calling one of them from cold works exactly like calling anyone
// already online.
...
// WebRTC handshake mailbox — see the file banner. `payload` carries an SDP blob (offer/answer) or
// a single ICE candidate; stored as opaque JSON, never inspected server-side.
