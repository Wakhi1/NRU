// Global VoIP calling engine — ported verbatim from SPTS's own voip-call.js ("voice is carried on
// the same data bundle as everything else, no separate airtime line"). Loaded on every
// authenticated page (see the <script> tag next to shell.js in each .html file) so a call can ring
// no matter which screen someone is on — the same way a phone rings regardless of what app is open.
// shell.js calls VoipCall.start() once, right after it knows who's signed in.
//
// Signaling is short-polling against /api/v1/voip (see voip.routes.js) — no websocket server. Once
// the SDP offer/answer and ICE candidates have crossed, audio flows peer-to-peer over WebRTC
// directly; the server never touches the call itself, only the handshake.
const VoipCall = (() => {
  ...
  // Floating quick-dial button — present on every authenticated page (gated by shell.js the same
  // way the rest of VoipCall is), so a call can be placed without navigating to /voip.html first.
  // Fully wired to the same placeCall() the dedicated Calls page uses, not a shortcut to a stub.
  function ensureFab() {
  ...
  // Every active employee is listed here, not just people who've already opened FLMS — someone with
  // no extension yet still shows up (calling them provisions one on the spot, see voip.routes.js's
  // POST /calls), so the full staff list is reachable from day one.
