// No password hashing here — FLMS holds no local credential to hash or verify. Login is fully
...
async function getEffectiveRoleKeys(employeeNo) {
  ...
  return rows[0] && rows[0].role_name ? [rows[0].role_name] : [];
}
...
function requireScreen(screenKey) {
  ...
  return (req, res, next) => {
    ...
    const allowed = roles.some((k) => ((ROLES[k] && ROLES[k].screens) || []).includes(screenKey));
...
function requirePermission(permKey) {
  ...
  return (req, res, next) => {
    if (!req.session.user) return next(unauthorized());
    const roles = req.session.user.roleKeys || [];
    const allowed = roles.some((k) => ((ROLES[k] && ROLES[k].permissions) || []).includes(permKey));
