// Login is fully delegated to the HRIS — FLMS holds no local password, no local TOTP secret, no
// local email-OTP state for anyone, exactly the same explicit decision SPTS made ("there should be
// one database that records if a user has enrolled in an authenticator app — the HRIS"). Every
// employee in this ecosystem is the same account everywhere; FLMS's job is to ask the HRIS "is this
// login valid?" and "does this person need a second factor?", never to keep its own copy of the
// answer. An employee with no HRIS login simply cannot sign in to FLMS — that's the correct outcome
// of centralizing identity, not a bug.
...
router.post('/login', asyncHandler(async (req, res) => {
  ...
  // then have nothing for FLMS to attach a role to. Rather than surface that as a confusing
  ...
  if (!emp[0] || emp[0].status !== 'active') {
    throw forbidden('Your HRIS account checked out, but FLMS hasn\'t synced you yet — ask IT to run "Reconcile from HRIS now"');
  }
  ...
  const methods = [];
  if (hrisStatus && hrisStatus.totp_enabled) methods.push('totp');
  if (hrisStatus && hrisStatus.email_otp_enabled) methods.push('email');
...
router.get('/me', requireAuth, asyncHandler(async (req, res) => {
  ...
  res.json({
    data: {
      ...
      photoPath: (photoRows[0] && photoRows[0].photo_path) || null,
      roleKeys,
      roleLabels: roleKeys.map((k) => ROLES[k] && ROLES[k].label).filter(Boolean),
      screens: screensFor(roleKeys),
      permissions: Array.from(new Set(roleKeys.reduce((acc, k) => acc.concat((ROLES[k] && ROLES[k].permissions) || []), []))),
    },
  });
}));
