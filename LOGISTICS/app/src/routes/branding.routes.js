// Org branding (logo, favicon, org name) — mirrors SPTS's own branding.routes.js pattern, which
// itself mirrors the HRIS's. Mounted WITHOUT requireAuth: the login page and the app shell both
// need to render the current logo before a session exists. GET / is fully public; the two upload
// endpoints do their own admin check inline since the router isn't wrapped.
...
// throughout this app (see platform/scope.js — roles are the HRIS's own, not FLMS-invented ones).
